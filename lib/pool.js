...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SubmitBlock(blockHex, finishedCallback){
        ...
        emitLog('RPC arguments:' + JSON.stringify(rpcArgs));
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var auxResult = function(hash, aux) {
                if(!hash) return;
                CheckBlockAccepted(hash, blockHex, _this.auxes[aux].daemon, function (accepted, tx, height, value) {
            ...
            */
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function () {
                    ...
                    emitInfoLog('shareData.blockHash: ' + shareData.blockHash);

                    CheckBlockAccepted(shareData.blockHash, blockHex, _this.daemon, function (isAccepted, tx, height, value) {
    ...
    function CheckBlockAccepted(blockHash, blockHex, daemon, callback){
        //setTimeout(function(){
        emitLog("CheckBlockAccepted getblock hash: " + JSON.stringify(blockHash));
            daemon.cmd('getblock',
                [blockHash],
                function (results) {
                    emitLog("CheckBlockAccepted getblock results: " + JSON.stringify(results));
